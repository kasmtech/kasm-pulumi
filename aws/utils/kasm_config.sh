...
response=$(curl "https://$URL/api/admin/create_server_pool"   -k   -s   -H 'accept: application/json'   -H 'content-type: application/json'   -b "username=admin@kasm.local; session_token=$KASM_TOKEN"   --data-raw '{"target_server_pool":{"server_pool_name":"aws_autoscaler_pool","server_pool_type":"Docker Agent"},"token":"'"$KASM_TOKEN"'","username":"admin@kasm.local"}');       > /dev/null 2>&1;
...
aws_info=$(echo "$AWS_INFO")
agent_disk_size=$(echo "$aws_info" | jq -r '.aws_ec2_ebs_volume_size_gb')
aws_ec2_iam=$(echo "$aws_info" | jq -r '.aws_ec2_iam')
...
echo "$ZONE_INFO" | jq -c '.zones[]' | while read -r zone; do
  ...
  if [ "$zone_name" != "default" ]; then
    zone_info=$(echo "$AWS_INFO" | jq -r --arg name "$zone_name" '.["additional_zone"][0][] | select(.name == $name)')
    subnet=$(echo "$zone_info" | jq -r '.aws_ec2_subnet_id')
    max_instance=$(echo "$zone_info" | jq -r '.["max_instance"]')
    agent_size=$(echo "$zone_info" | jq -r '.aws_ec2_instance_type')
    region=$(echo "$zone_info" | jq -r '.region')
    sg=$(echo "$zone_info" | jq -r '.aws_ec2_security_group_ids')
    aws_ec2_ami_id=$(echo "$zone_info" | jq -r '.aws_ec2_ami_id')
  else
    subnet=$(echo "$aws_info" | jq -r '.aws_ec2_subnet_id')
    max_instance=$(echo "$aws_info" | jq -r '.["max_instance"]')
    agent_size=$(echo "$aws_info" | jq -r '.aws_ec2_instance_type')
    region=$(echo "$aws_info" | jq -r '.region')
    sg=$(echo "$aws_info" | jq -r '.aws_ec2_security_group_ids')
    aws_ec2_ami_id=$(echo "$aws_info" | jq -r '.aws_ec2_ami_id')
  fi
  ...
  response=$(curl "https://$URL/api/admin/create_vm_provider_config"   -k   -s   -H 'accept: application/json'   -H 'content-type: application/json'   -b 'username="admin@kasm.local"; session_token="'"$KASM_TOKEN"'"'   --data-raw '{"target_vm_provider_config":{"vm_provider_config_name":"'"$zone_name"'_aws_provider_config","vm_provider_name":"aws","max_instances":"'"$max_instance"'","vm_installed_os_type":"linux","startup_script_type":"startup-script","startup_script":"'"$script_body"'","aws_region":"'"$region"'","aws_ec2_instance_type":"'"$agent_size"'","aws_ec2_ami_id":"'"$aws_ec2_ami_id"'", "aws_ec2_iam": "'"$aws_ec2_iam"'","aws_ec2_ebs_volume_size_gb":"'"$agent_disk_size"'","aws_ec2_ebs_volume_type":"gp3","aws_ec2_subnet_id":"'"$subnet"'", "aws_ec2_security_group_ids": '"$sg"', "aws_ec2_custom_tags": "{}", "aws_ec2_config_override": "{}", "aws_access_key_id": "REPLACE_ME_AND_ACCESS_KEY", "aws_secret_access_key": " ", "aws_ec2_private_key": "'"$SSH_PRIVATE_KEY_PEM"'"},"token":"'"$KASM_TOKEN"'","username":"admin@kasm.local"}');       > /dev/null 2>&1;  vm_provider_config_id=$(echo "$response" | jq -r '.vm_provider_config.vm_provider_config_id');
